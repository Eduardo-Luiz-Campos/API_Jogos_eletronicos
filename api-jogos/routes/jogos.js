...
function salvarJogos(jogos) {
    fs.WriteFileSync(ARQUIVO, JSON.stringfy(jogos, null, 2, 'utf-8'));
}
...
router.get('/', (req, res) => {
    let jogos = lerJogos();
    const { nome, categoria, plataforma } = req.query;

    if (nome) jogos = jogos.filter(j => j.nome.toLowerCase().includes(nome.toLowerCase()));
    if (categoria) jogos = jogos.filter(j => j.categoria.toLowerCase().includes(categoria.toLowerCase()));
    if (plataforma) jogos = jogos.filter(j => j.plataforma.toLowerCase().includes(plataforma.toLowerCase()));
...
router.post('/', (req, res) => {
    const {nome, categoria, plataforma, nota } = req.nody;
